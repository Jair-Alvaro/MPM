      ...
      <UDashboardPanel grow>
        ...
        <UDashboardPanelContent>
          <div class="grid grid-cols-3 gap-4 items-center">
            <!-- Filtro por activos -->
            <div>
              <select v-model="selectedActivo" @change="filterSubconjuntos">
                <option value="">Todos los Activos</option>
                <option v-for="activo in activos" :value="activo.id" :key="activo.id">{{ activo.nombre }}</option>
              </select>
            </div>
            <!-- Botón de exportar -->
            <button @click="exportToExcel" class="export-button bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded">
              Exportar
            </button>
          </div><br />
            ...
            <table class="table-auto w-full rounded border border-green-500">
              <thead>
                <tr>
                  ...
                  <th class="px-4 py-2 border border-green-500">Número de Pieza</th>
                  <th class="px-4 py-2 border border-green-500">Número de Serie</th>
                  <th class="px-4 py-2 border border-green-500">Código</th>
                  <th class="px-4 py-2 border border-green-500">Imagen</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="subconjunto in filteredSubconjuntos" :key="subconjunto.id">
                  <td class="border px-4 py-2">{{ subconjunto.nombre }}</td>
                  <td class="border px-4 py-2">{{ subconjunto.descripcion }}</td>
                  <td class="border px-4 py-2">{{ subconjunto.fabricante }}</td>
                  <td class="border px-4 py-2">{{ subconjunto.modelo }}</td>
                  <td class="border px-4 py-2">{{ subconjunto.numero_pieza }}</td>
                  <td class="border px-4 py-2">{{ subconjunto.numero_serie }}</td>
                  <td class="border px-4 py-2">{{ subconjunto.codigo }}</td>
                  <td class="border px-4 py-2">
                    <img :src="getSmallImage(subconjunto.imagen)" alt="Imagen del Subconjunto" class="w-24 h-auto" />
                    <p v-if="!subconjunto.imagen" class="text-gray-500">Sin imagen</p>
                  </td>
                </tr>
              </tbody>
            </table>
...
const filterSubconjuntos = () => {
  filteredSubconjuntos.value = subconjuntos.value.filter(subconjunto => {
    return !selectedActivo.value || subconjunto.id_activo === selectedActivo.value;
  });
};
